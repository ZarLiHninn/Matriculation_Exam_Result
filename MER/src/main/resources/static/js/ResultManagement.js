...
document.getElementById("rollno").onfocus=function(){
		$('#selectrollno').empty();
		...
		//var urlink='http://'+host+'/'+path+'/LGN002/selectedyear/'+yeardiv;
		var urlink='http://'+host+'/LGN002/selectedyear/'+yeardiv;
		 $.ajax({
		        type : 'GET',
		        url : urlink,
		        dataType : 'json',
...
document.getElementById("rollno").onfocusout=function() {
	...
	if(rollnodiv==""){
		...
		var host=window.location.host;
		var path=location.pathname.split('/')[1];
		//var urlink1='http://'+host+'/'+path+'/LGN002/selectedyear/'+yeardiv+'/'+rollnodiv;
		var urlink1='http://'+host+'/LGN002/selectedyear/'+yeardiv+'/'+rollnodiv;
	 $.ajax({
	        type : 'GET',
	        url : urlink1,
	        dataType : 'json',
...
function changeDatalist2(data){
	...
	if(datastring==undefined){
		document.getElementById("errorForStudentName").innerText="Student Name doesn't exist.";
		document.getElementById("stdName").value="";
		document.getElementById("stdId").value="";
	}
	else{
...
function deletefunction(id){
	var host=window.location.host;
	var path=location.pathname.split('/')[1];
	//var urlink='http://'+host+'/'+path+"/LGN002/deleteResult/"+id;
	var urlink='http://'+host+"/LGN002/deleteResult/"+id;
	bootbox.confirm({
...
function markCharacterType(){
	var markAllId=document.querySelectorAll("[id='Mark']");
	for(var i=0;i<markAllId.length;i++){
		var regex=/[^0-9]/gi;
		markAllId[i].value=markAllId[i].value.replace(regex,"");
	}
}
